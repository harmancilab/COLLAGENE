																...
																vector<t_annot_region*>* promotor_regs, int l_promotor)
{
	...
	for(int i_reg = 0; i_reg < (int)gff_regs->size(); i_reg++)
	{
		...
		for(int i_t = 0; i_t < (int)cur_reg_toks->size(); i_t++)
		{
	...
	for(int i_r = 0; i_r < (int)transcript_regs->size(); i_r++)
	{
	...
	for(int i_r = 0; i_r < (int)gene_regs->size(); i_r++)
	{
	...
	for(int i_cds = 0; i_cds < (int)cds_regs->size(); i_cds++)
	{
		t_CDS_annotation_info* cur_cds_info = (t_CDS_annotation_info*)(cds_regs->at(i_cds)->annotation_info);

		cur_cds_info->transcript = NULL;

		int trans_i = t_string::fast_search_string_per_prefix(cds_regs->at(i_cds)->name, sorted_transcript_names, 0, (int)sorted_transcript_names->size()-1);

		while(trans_i > 0 &&
			trans_i < (int)sorted_transcript_names->size() &&
		...
		{
			fprintf(stderr, "Could not assign the transcript for %s (%d); %s\n", cds_regs->at(i_cds)->name, trans_i, sorted_transcript_names->at(trans_i));
			getc(stdin);
		}
	} // i_cds loop.

	// Filter transcripts that are weird, paralogs, etc.
	vector<t_annot_region*>* filtered_transcript_regs = new vector<t_annot_region*>();
	fprintf(stderr, "Filtering %d transcripts per paralogs, etc..\n", (int)transcript_regs->size());
	for (int trans_i = 0; trans_i < (int)transcript_regs->size(); trans_i++)
	{
		vector<t_annot_region*>* transcript_cds_regs = ((t_transcript_annotation_info*)(transcript_regs->at(trans_i)->annotation_info))->CDSs;

		if (transcript_cds_regs->size() > 0)
		{
			bool passed_chr_check = true;
			for (int i_cds = 0; i_cds < (int)transcript_cds_regs->size(); i_cds++)
			{
				if (!t_string::compare_strings(transcript_cds_regs->at(i_cds)->chrom, transcript_regs->at(trans_i)->chrom))
				{
					passed_chr_check = false;
					break;
				}
			} // i_cds loop.
			...
			if (passed_chr_check)
			{
				filtered_transcript_regs->push_back(transcript_regs->at(trans_i));
			}
		} // cds size check.
	} // trans_i loop.
	fprintf(stderr, "Filtered to %d transcripts..\n", (int)filtered_transcript_regs->size());
	transcript_regs = filtered_transcript_regs;

	// Get the intronic regions: The intronic regions exclude the regions where we see an exon for ANY gene.
	fprintf(stderr, "Generating the intronic regions per transcript.\n");
	delete_annot_regions(exon_regs);
	for (int trans_i = 0; trans_i < (int)transcript_regs->size(); trans_i++)
	{
		vector<t_annot_region*>* transcript_cds_regs = ((t_transcript_annotation_info*)(transcript_regs->at(trans_i)->annotation_info))->CDSs;
		...
		if (transcript_cds_regs->size() > 0)
		{
			...
			for (int i_cds = 0; i_cds < (int)transcript_cds_regs->size() - 1; i_cds++)
			{
				int cur_intron_start = transcript_cds_regs->at(i_cds)->end + 1;
				int cur_intron_end = transcript_cds_regs->at(i_cds + 1)->start - 1;
				int l_intron = cur_intron_end - cur_intron_start + 1;

				// Report the transcripts with overlapping cds's
				if (MIN(transcript_cds_regs->at(i_cds)->end, transcript_cds_regs->at(i_cds + 1)->end) > MAX(transcript_cds_regs->at(i_cds)->start, transcript_cds_regs->at(i_cds + 1)->start))
				{
					fprintf(stderr, "%s has overlapping CDS: %d-%d, %d-%d\n",
							transcript_regs->at(trans_i)->name,
							transcript_cds_regs->at(i_cds)->start, transcript_cds_regs->at(i_cds)->end,
							transcript_cds_regs->at(i_cds + 1)->start, transcript_cds_regs->at(i_cds + 1)->end);
					//exit(0);
				}
				else
				{
					...
					if (l_intron > 10)
					{
						t_annot_region* cur_intronic_reg = get_empty_region();
						cur_intronic_reg->chrom = t_string::copy_me_str(transcript_cds_regs->at(i_cds)->chrom);
						cur_intronic_reg->start = cur_intron_start;
						cur_intronic_reg->end = cur_intron_end;
						cur_intronic_reg->strand = transcript_cds_regs->at(i_cds)->strand;

						t_intron_annotation_info* intron_info = new t_intron_annotation_info();
						intron_info->gene = (t_annot_region*)(transcript_cds_regs->at(i_cds));
						intron_info->gene_name = (intron_info->gene->name);
						cur_intronic_reg->name = t_string::copy_me_str(intron_info->gene->name);
						cur_intronic_reg->annotation_info = intron_info;
	...
	fprintf(stderr, "Extracted %d intronic regions.\n", (int)intronic_regs->size());

	fprintf(stderr, "Assigning the transcripts to the genes.\n");
	for(int i_tr = 0; i_tr < (int)transcript_regs->size(); i_tr++)
	{
		t_transcript_annotation_info* cur_tr_annot_info = (t_transcript_annotation_info*)(transcript_regs->at(i_tr)->annotation_info);

		int gene_i = t_string::fast_search_string_per_prefix(cur_tr_annot_info->gene_name, sorted_gene_names, 0, (int)sorted_gene_names->size()-1);
		cur_tr_annot_info->gene = NULL;

		while(gene_i > 0 &&
			gene_i < (int)sorted_gene_names->size() &&
	...
	for(int i_chr = 0; i_chr < (int)cds_chr_ids->size(); i_chr++)
	{
		...
		for(int i_cds = 0; i_cds < (int)cur_chr_cds_regs->size(); i_cds++)
		{
		...
		for(int i_int = 0; i_int < (int)cur_chr_intron_regs->size(); i_int++)
		{
															...
															char* source_str)
{
	...
	for(int i_reg = 0; i_reg < (int)all_regions->size(); i_reg++)
	{
...
void parse_group_strs(vector<t_annot_region*>* all_regions, void (*parse_grp_str)(t_annot_region*, vector<char*>*, vector<char*>*))
{
	for(int i_r = 0; i_r < (int)all_regions->size(); i_r++)
	{
		...
		{
			fprintf(stderr, "Group string parsing failed: %d, %d\n", (int)cur_reg_prop_ids->size(), (int)cur_reg_prop_vals->size());
			exit(0);
		}
...
char* get_prop_val_per_prop_id(t_annot_region* region, char* prop_id)
{
	...
	for(int i_id = 0; i_id < (int)prop_ids->size(); i_id++)
	{
											...
											char* prop_id)
{
	...
	for(int i_reg = 1; i_reg < (int)string_sort_idx->size(); i_reg++)
	{
		if(i_reg % 10000 == 0)
		{
	...
	for(int i_reg = 0; i_reg < (int)sel_regions_lists->size(); i_reg++)
	{
		if(i_reg % 1000 == 0)
		{
			fprintf(stderr, "Forming parent regions %d(%d)       \r", i_reg, (int)sel_regions_lists->size());
		}
		...
		for(int i_sg = 0; i_sg < (int)sel_regions_lists->at(i_reg)->size(); i_sg++)
		{
		...
		fprintf(stderr, "Setup the parent region %s: %d\n", sel_region_props->at(i_reg), (int)new_merged_region->intervals->size());
	...
	char* (*get_parent_feature_id_per_parent_group_str)(t_annot_region*)) // Use this to extract the id for the parent using the group string for parent.
{
	...
	for(int i_p = 0; i_p < (int)parent_regions->size(); i_p++)
	{
	...
	for(int i_c = 0; i_c < (int)children_regions->size(); i_c++)
	{
	...
	for(int i_c = 0; i_c < (int)children_regions->size(); i_c++)
	{
		if(i_c % 1000 == 0)
		{
			fprintf(stderr, "Processing %d. child. (%d)            \r", i_c, (int)children_regions->size());
		}
		...
		for(int i_c_p = 0; i_c_p < (int)cur_child_parent_ids->size(); i_c_p++)
		{
			bool found_parent = false;
			for(int i_p = 0;
				!found_parent && i_p < (int)parent_regions->size();
				i_p++)
			{
...
t_annot_region* copy_gff_entry(t_annot_region* gff_entry)
{
	...
	for(int i_prop = 0; i_prop < (int)orig_gff_info->prop_ids->size(); i_prop++)
	{
